...
// things that could be configurable options but are here as constants instead
	...
	this.slowZoomImage = 'loading_white.gif';  // from the urlGraphics folder, overlays the zoomDiv image
	this.slowAnchorImage = 'loading_black.gif';  // from the urlGraphics folder, appears above the clicked anchor
	this.resizeUpCursor = 'magnify_plus.cur';  // from the urlGraphics folder, custom cursor appears above the image
	this.resizeDownCursor = 'magnify_minus.cur';
...
// things that are present only on the primary floatbox
		...
		// also for english ie because ie won't display the arrows without language pack installs
		this.strings = {
...
// browser detects
	...
	/*@cc_on
		...
		this.ieNew = !!window.postMessage;  // ie8beta detect - probably not needed post-release
	@*/
	} else if (navigator.userAgent.indexOf('Firefox') !== -1) {
		this.ff = true;
		this.ffNew = !!document.getElementsByClassName;  // v3+
		this.ffOld = !this.ffNew;
	}
	...
	if (this.rex.fbxd.test(a.rel)) {  // if one of the "pick me" strings is on the rel attribute...
		...
		if (i === -1) {  // not already in the anchor array
			...
			if (typeOverride === 'img' || this.rex.img.test(a.href)) { // image?
				...
				var doc = this.doc;  // in iframes, which doc do we look for inline divs in?
				...
				var el, match = this.rex.div.exec(a.href);
				if (match && (el = doc.getElementById(match[1]))) {  // inline element?
					a.type = 'inline';
...
objectHTML: function(href, type) {
	// unfortunately, yahoo videos won't play in firefox 3+ from an object, only from an embed
	// this first section is a shameless hack to make that combination work
	if (this.ffNew && type === 'flash' && /\bYV_YEP\.swf\b/i.test(href)) {
		return '<embed src="' + this.encodeHTML(href) + '" type="application/x-shockwave-flash"' +
		' width="%width%" height="%height%" flashVars="autoplay=1"></embed>';
	}
	...
	if (type === 'flash') {
		var classid = 'classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"',
			...
			flashvars:'autoplay=1&amp;ap=true&amp;border=0&amp;rel=0' };  // autoplay for youtube, yahoo and google. ap for msn. border and rel(ated) for youtube
	} else {  // quicktime
...
start: function(anchor) {
	...
	if (this.isRestart) {
		...
		top.location.replace = function() { return false; };  // frame buster buster (doesn't work in firefox or safari)
	}
...
// 				<div id="fbLowerNav">
// 					<div id="fbLowerPrev">
// 						<a id="fbLowerPrevA" title="hintPrev"></a>
// 					</div>
// 					<div id="fbLowerNext">
// 						<a id="fbLowerNextA" title="hintNext"></a>
// 					</div>
// 				</div>
// 				<div id="fbControls">
...
//	</div>
	...
	this.fbLowerPrev	= this.newNode('div', 'fbLowerPrev', this.fbLowerNav);
	this.fbLowerPrevA	= this.newNode('a', 'fbLowerPrevA', this.fbLowerPrev, this.strings.hintPrev);
	this.fbLowerNext	= this.newNode('div', 'fbLowerNext', this.fbLowerNav);
	this.fbLowerNextA	= this.newNode('a', 'fbLowerNextA', this.fbLowerNext, this.strings.hintNext);
...
newNode: function(nodeType, id, parentNode, title) {
	...
	if (nodeType === 'a') {
		...
		node.src = 'javascript:("");';  // IE6 SSL bug fix
	}
...
// short-hand style closures for event handlers
	...
	lowerPrevA = this.fbLowerPrevA.style,
	lowerNextA = this.fbLowerNextA.style;
// utility function used by mouse events to clear show-once hints
	if (this.showHints === 'once') {
		this.hideHint = function(id) {
			if (that[id].title) {
				that.timeouts[id] = setTimeout(function() { that[id].title = ''; }, that.showHintsTime);
			}
		};
	} else {
...
// attach event behaviours to the controls
	...
	this.fbLowerPrevA.onclick = function(step) {
	...
	this.fbLowerNextA.onclick = function(step) {
		if (typeof step !== 'number') step = 1;
...
// set nav panel mouse actions
	...
	this.fbUpperPrev.onmousemove = function() {
		...
		if (that.lowerNav) lowerPrevA.backgroundPosition = that.onPos;
	...
	this.fbUpperNext.onmousemove = function() {
		...
		if (that.lowerNav) lowerNextA.backgroundPosition = that.onPos;
		return true;
	};
	...
	this.fbLeftNav.onmouseout = function() {
		...
		if (that.lowerNav) lowerPrevA.backgroundPosition = that.offPos;
	};
	this.fbRightNav.onmouseout = function() {
		upperNext.visibility = 'hidden';
		if (that.lowerNav) lowerNextA.backgroundPosition = that.offPos;
	};
...
keyboardAction: function() {
	...
	// left/right arrow: prev/next item
			case 37: case 39:
				if (that.itemCount > 1) {
					that[keyCode === 37 ? 'fbLowerPrevA' : 'fbLowerNextA'].onclick((e.ctrlKey || e.metaKey) ? that.ctrlJump : 1);
					if (that.showHints === 'once') {
						// turn off hints, because user already knows
						that.fbLowerPrevA.title = that.fbLowerNextA.title = '';
						if (that.upperNav) that.fbUpperPrev.title = that.fbUpperNext.title = '';
					}
				}
...
// set starting dimensions
	...
	if (anchorPos.width) {  // it's a thumbnail and we're zooming from it
		...
		var slowLoad = function() {
			that.fbZoomImg.src = that.urlGraphics + that.slowZoomImg;
		...
		var slowLoad = function() {
			...
			that.fbZoomImg.src = that.urlGraphics + that.slowAnchorImg;
			zoomDiv.display = zoomImg.display = '';
		};
	}
...
// configure elements
	...
	if (this.dropShadow && !this.ieOld) {  // ie6 can't handle transparent pngs
...
// lowerNav
	if (this.lowerNav) {
		...
		lowerPrev.backgroundPosition = lowerPrevA.backgroundPosition =
		lowerNext.backgroundPosition = lowerNextA.backgroundPosition = this.offPos;
		lowerNav.paddingRight = this.controlSpacing + 'px';
		controlPanel.display = lowerNav.display =
		lowerPrev.display = lowerPrevA.display =
		lowerNext.display = lowerNextA.display = '';
	}
// controls
	var width = 0;
	if (this.showClose) {
	...
	if (/img|iframe/.test(item.type)) {
		...
		if (item.type === 'img') {
			...
			loader.onerror = function() {  // if the image can't be found
				// show the 404 image
				if (item.href !== that.urlGraphics + '404.jpg') {
					this.src = item.href = that.urlGraphics + '404.jpg';
				}
			};
		...
		if (item.type === 'inline') {
			...
			this.fbDiv.appendChild(el);  // stick the clone in our div
		} else if (item.type === 'ajax') {
			this.setInnerHTML(this.fbDiv, '<p style="color:#000; background:#fff; margin:1em; padding:1em;">' +
			...
			this.xhr.getResponse(item.href, function(xhr) {
				if (xhr.responseText) that.setInnerHTML(that.fbDiv, xhr.responseText);
			});
		}
...
// add screen scroll values to left and top
	if (!this.ieNew) box.display = 'none';  // fb might be stretching the body (test and probably remove the ieNew check when IE8 is released)
	if (this.ieOld) this.stretchOverlay()();  // the overlay might be stretching the body
	...
	if (this.currentItem.type === 'img' ? this.resizeImages : this.resizeOther) {
		...
		if (scale) {  // either direction
			...
			if (this.currentItem.type === 'img' && !this.opera && /cursor|both/.test(this.resizeTool)) {
				...
				this.fbContent.style.cursor = 'url(' + this.urlGraphics + (scale === -1 ? this.resizeDownCursor : this.resizeUpCursor) +'), default';
...
// if a scrollbar has come or gone, we might need some further resizing or positioning done
		if (!this.resized) {  // once only thanks
			display = this.getDisplaySize();
			var vscrollChanged = display.width !== this.displaySize.width,
				hscrollChanged = display.height !== this.displaySize.height,
				tolerance = 25 + 2*this.shadowSize;
			if ((vscrollChanged && Math.abs(this.pos.fbBox.width - display.width) < tolerance) ||
			(hscrollChanged && Math.abs(this.pos.fbBox.height - display.height) < tolerance)) {
				this.resized = true;
...
// fetch content that was deferred
		if (this.fbContent && (!this.fbContent.src || this.fbContent.src === 'javascript:("");')) {
...
// toggle nav gadgets based on wrap status & update nav hrefs (for the browser status bar display)
		if (this.lowerNav) {
			if (!this.operaOld) {  // opera prior to 9.5 gets nothing because of stupid big href url tooltips
...
// show the drop shadow if it's not already shown
	if (this.dropShadow && this.fbShadowRight.style.display && !this.ieOld) {
...
// activate any floatboxed links in the box (includes infoPanel but does not include iframe content)
	this.tagAnchors(this.fbBox);
...
// show the overlay and animate zoomDiv up to the size and position where the content div will be
		...
		return this.fadeOpacity(this.fbOverlay, this.overlayOpacity, this.overlayFadeDuration, setSize);
	}
	if (phase === 1) {
// animate box from exactly behind zoomDiv to its final size
		...
		var zoomIn = function() { that.zoomIn(2); },
			restore = function() { that.restore(zoomIn); };
		return this.setSize(
...
// place zoomDiv over top of current content
		...
		var pad = this.outerBorder + this.padding + this.innerBorder - this.zoomPopBorder;
		return this.setSize(
			{ id: 'fbZoomDiv', left: this.pos.fbBox.left + pad, top: this.pos.fbBox.top + pad,
			width: this.pos.fbDiv.width, height: this.pos.fbDiv.height },
...
// we're done. clear ie filter and run on-complete code
		if (this.ie && finishOp >= 1) el.style.removeAttribute('filter');  // fix for IE alpha opacity filter bug
		if (callback) callback();
	} else {
...
getLeftTop: function(el, local) {
	var left = el.offsetLeft || 0,
		...
		elFlow = !rex.test((el.currentStyle && el.currentStyle.position) ||
		(compStyle && compStyle(el, '').getPropertyValue('position'))),
		inFlow = elFlow,
		node = el;
	while ((node = node.offsetParent)) {  // for each offset parent
		var borderLeft = 0, borderTop = 0, nodeFlow = true;
		if (node.currentStyle) {  // IE & Opera
			nodeFlow = !rex.test(node.currentStyle.position);
			if (this.opera) {  // Opera
		...
		} else if (compStyle) {  // FF & Safari
			nodeFlow = !rex.test(compStyle(node, '').getPropertyValue('position'));
			// borders to be added in later (not for local position)
	...
	if (!local && win !== this.win) {  // if el is in an iframe, find it
		...
		while (i--) {
			...
			try {  // in case of cross-domain script blocking
				...
				if (idoc.document) idoc = idoc.document;  // now it's the doc for sure
			} catch(e) {}
			...
			if (idoc === doc || (typeof idoc !== 'object' && rex.test(win.location))) {  // if this iframe contains the element we are measuring...
				...
				if (node.currentStyle) {
					...
					// add iframe border and padding for FF and Safari
		...
		getResponse: function(url, callback) {
			try {
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 203 || xhr.status === 304)) {
						xhr.onreadystatechange = function() {};
						callback(xhr);
					}
				};
...
function initfb() {
	...
	if (document.compatMode === 'BackCompat') {
		...
		alert('Floatbox does not support quirks mode.\n Add a valid doc type to your page.');
		return;
	}
