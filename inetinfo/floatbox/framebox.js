/************************************************************************************************
* Floatbox v3.10  (frame constrained version)
* September 01, 2008
*
...
init: function() {
	...
	this.slowZoomImage = 'loading_white.gif';
	this.slowAnchorImage = 'loading_black.gif';
	this.resizeUpCursor = 'magnify_plus.cur';
	...
	/*@cc_on
		...
		this.ie = true;
		this.ieOld = @_jscript_version < 5.7;
		this.ieNew = !!window.postMessage;
	@*/
	} else if (navigator.userAgent.indexOf('Firefox') !== -1) {
		this.ff = true;
		this.ffNew = !!document.getElementsByClassName;
	...
	if (this.rex.fbxd.test(a.rel)) {
		...
		if (i === -1) {
			...
			if (typeOverride === 'img' || this.rex.img.test(a.href)) {
				...
				if (match && (el = doc.getElementById(match[1]))) {
					a.type = 'inline';
...
calcSize: function(fit, recalc) {
	...
	if (!this.ieNew) box.display = 'none';
